    ...
    public class ReviewsController
    {
        ...
        public async Task<ActionResult> GetReviews(int id)
        {
            ...
            var review = await _context.Reviews
             .Where(r => r.Id == id)
                 .Include(r => r.ReviewItems) //join table ReviewsItems
                    .ThenInclude(ri => ri.Car) //then join table Car
                        .ThenInclude(car => car.Model) //then join table Model
             .Select(r=> new ReviewDetailDTO(r.Id,r.Country, r.DriverType.ToString(), r.ReviewItems
                        .Select(ri => new ReviewlItemDTO(ri.CarId,ri.Car.Model,ri.Car.Color,ri.Car.FuelType,ri.Rating,ri.Car.Description).ToList<ItemDTO>()))
             .FirstOrDefaultAsync();
